...
from app.operations import (add_sponsor_to_event, create_event, create_sponsor,
                            create_ticket, delete_ticket,
                            get_all_tickets_for_show, get_ticket,
                            update_ticket, update_ticket_price)
...
async def read_ticket(
    db_session: Annotated[AsyncSession, Depends(get_db_session)], ticket_id: int
):
...
@app.post(
    ...
    responses={
        200: {
            ...
            "content": {"application/json": {"example": {"sponsor_id": 12345}}},
        }
    },
)
...
async def register_sponsor_amount_contribution(
    ...
    registered = await add_sponsor_to_event(db_session, event_id, sponsor_id, amount)
    if not registered:
        raise HTTPException(
            status_code=400,
