...
async def get_ticket(db_session: AsyncSession, ticket_id: int) -> Ticket | None:
    query = (
        select(Ticket).where(Ticket.id == ticket_id).options(joinedload(Ticket.details))
    )
...
async def get_all_tickets_for_show(db_session: AsyncSession, show: str) -> list[Ticket]:
    async with db_session as session:
        result = await session.execute(select(Ticket).where(Ticket.show == show))
...
async def create_event(
    ...
    async with db_session.begin():
        ...
        tickets = [
            Ticket(
                show=event_name, details=TicketDetails(seat=f"{n}A"), event_id=event_id
            )
...
async def get_event(db_session: AsyncSession, event_id: int) -> Event | None:
    query = (
