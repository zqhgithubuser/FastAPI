...
def test_client_update_ticket_price(test_client, add_special_ticket):
    request = test_client.put("/ticket/1234/price/250.0")
    assert request.status_code == 200
    assert request.json() == {"detail": "Price updated"}


def test_client_update_ticket(test_client, add_special_ticket):
    request = test_client.put("/ticket/1234", json={"price": 250.0})
    assert request.status_code == 200
    assert request.json() == {"detail": "Ticket updated"}


def test_client_delete_ticket(test_client, add_special_ticket):
...
def test_client_get_all_tickets_for_concert(test_client, fill_database_with_tickets):
    request = test_client.get("/tickets/The Rolling Stones Concert")
    assert request.status_code == 200
    assert request.json() == [
        {
            "id": id_,
