...
from app.operations import (add_sponsor_to_event, create_event, create_ticket,
                            delete_ticket, get_all_tickets_for_show, get_event,
                            get_event_sponsorships_with_amount,
                            get_events_tickets_with_user_price, get_ticket,
                            sell_ticket_to_user, update_ticket,
                            update_ticket_details, update_ticket_price)
...
async def test_delete_ticket(add_special_ticket, db_session_test):
    assert await delete_ticket(db_session_test, 5555) is False  # non existing ticket
    assert await delete_ticket(db_session_test, 1234) is True  # existing ticket
    await assert_tickets_table_is_empty(db_session_test)
...
async def test_update_ticket_sell_to_user(add_special_ticket, db_session_test):
...
async def test_ticket_cannot_sell_sold_ticket(add_special_sold_ticket, db_session_test):
...
async def test_register_sponsorship(add_event_and_sponsor, db_session_test):
    await add_sponsor_to_event(db_session_test, 1, 1, 100)
    async with db_session_test as session:
        result = await session.execute(
            select(Event).options(joinedload(Event.sponsors))
        )
        event = result.scalars().first()
...
async def test_update_sponsorship_amount(
    ...
    await add_sponsor_to_event(db_session_test, 1, 1, 200)
    async with db_session_test as session:
        result = await session.execute(select(Sponsorship))
